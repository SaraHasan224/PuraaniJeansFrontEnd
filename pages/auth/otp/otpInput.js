		...
		return (
			<div className={className} style={{ display: 'flex', alignItems: 'center' }}>
				<input
					...
					autoComplete="off"
					style={Object.assign(
						{ width: '70%', textAlign: 'center', height: '3rem' },
...
class OtpInput extends Component {
	...
	handleOtpChange = (otp, isClipboard) => {
	...
	isInputValueValid = (value) => {
		const isTypeValid = this.props.isInputNum
			...
			: typeof value === 'string'
		return (
			(isTypeValid && value.trim().length === 1) ||
			//android clipboard 6 characters allowed (maxlength 6)
			(isTypeValid && value.trim().length === 6)
		)
	}
	...
	changeCodeAtFocus = (value) => {
		...
		if (value.length === 6) {
			return this.handleOtpChange(value, true)
		} else {
	...
	handleOnPaste = (e) => {
		...
		const otp = this.getOtpValue()
		//resolved issue if paste on middle then letters removed
		if (activeInput > otp.length) {
			activeInput = otp.length
		}
